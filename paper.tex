...
The sample is predominantly Colombian (82\%), with the remainder drawn from
...
gender ($p = 0.31$), task score ($p = 0.30$),  nationality ($p = 0.15$)  
and education ($p = 0.55$), confirming that randomisation was successful.
Unfortunately, the nationality ($p = 0.15$) though non significant under the conventional threshold, 
is the least balanced variable, with the below group having a higher share of Colombians than the above group. 
This imbalance is likely due to chance given the small sample size and might have some implications for the results.
...
treatment group.
The pattern is monotone: participants told they outperformed their matched
peer attribute the most to effort ($\overline{EL}_{\text{above}} = 7.56$,
SD = 2.31), followed by the no-feedback group
($\overline{EL}_{\text{nofb}} = 7.00$, SD = 2.03) and the below group
($\overline{EL}_{\text{below}} = 6.35$, SD = 2.58).
...
the above group (SE = 0.51, $p < 0.05$), a large and statistically significant
difference.
The no-feedback group scores 0.56 points lower (SE = 0.44), a difference in
the expected direction that falls short of conventional significance.
Both estimates are stable once controls for age, gender, nationality, task
score, and education are added (column~2): the below-group gap widens slightly
to 1.29 points (SE = 0.54, $p < 0.05$), confirming that the pattern does
not reflect pre-existing compositional differences across groups.
Three participants did not complete the effort--luck slider; columns 1--2
use $N = 143$.

The pattern is consistent with the motivated-reasoning account of
\citet{Kunda1990}: when individuals receive a comparative performance signal,
they do not update their beliefs symmetrically.
Learning one outperformed a peer appears to activate a directional reasoning
process that reinforces the inference that effort, rather than chance,
determines outcomes --- while learning one underperformed does not produce
a symmetric shift toward luck-based attribution relative to having received
no information at all.
This asymmetry echoes the well-documented positivity bias in self-serving
attribution documented by \citet{Mezulis2004}, whereby positive self-relevant
signals are more readily internalised than negative ones.
Crucially, because the comparative signal is exogenously assigned and
uncorrelated with actual task performance, these belief differences provide
direct experimental support for the channel theorised by \citet{Deffains2016}:
it is the \emph{perception} of relative standing, not objective performance
itself, that drives attribution.
...
(SE = \$0.28, $p < 0.10$).
Adding controls for age, gender, task score, and education leaves both estimates stable
