...
The no-feedback arm was included to test whether the \emph{information
content} of the comparative signal drives the update, or whether simply
performing the task is sufficient.
Its intermediate position --- below the above group and above the below
group in both outcomes --- is consistent with information playing a role:
both comparative signals displace beliefs and redistribution away from the
uninformed baseline, but in opposite directions.
Neither treated group individually differs from the no-feedback anchor at
conventional significance levels; the statistically identifiable contrast
...
coefficients are equal ($F(1, 142) = 2.45$, $p = 0.12$), consistent with
both signals contributing symmetrically rather than one side driving the
above--below gap.
...
redistribute the same amount ($F(1, 142) = 1.25$, $p = 0.26$), consistent
with both comparative signals shifting redistribution away from the
no-feedback anchor in opposite directions.
