...
(SE = \$0.28, $p < 0.10$).
Adding demographic and performance controls leaves both estimates stable
...
As a placebo, columns 5--6 report the same specifications for the luck
scenario, where performance feedback is normatively irrelevant by design.
The below group does not differ significantly from the above group in the
luck scenario (column 5: \$0.48, SE = \$0.34, $p = 0.16$), consistent
with the attribution channel: the above--below gap in redistribution is
concentrated in the merit scenario, where the signal about relative
performance is directly relevant.
The no-feedback group redistributes significantly more than the above group
in the luck scenario as well (column 5: \$0.65, SE = \$0.33, $p < 0.05$),
which suggests the above group may hold somewhat lower redistribution
preferences overall; we return to this in the discussion.
...
group, and the above--below gap in redistribution is absent in the luck
scenario, lending qualified support to a merit-specific attribution channel.
